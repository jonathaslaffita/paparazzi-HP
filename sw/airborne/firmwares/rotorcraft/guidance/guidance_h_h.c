...
#ifndef GUIDANCE_SIDE_THRUST
#define GUIDANCE_SIDE_THRUST 0.35*MAX_PPRZ
#endif

#ifndef GUIDANCE_FORWARD_THRUST
#define GUIDANCE_FORWARD_THRUST 0
#endif
...
struct Int32Vect3 guidance_h_sideact;
...
static void guidance_h_traj_run(bool in_flight)
{
  ...
  guidance_h_sideact.x = GUIDANCE_FORWARD_THRUST + guidance_h_cmd_earth.x *5;
  guidance_h_sideact.y = GUIDANCE_SIDE_THRUST + guidance_h_cmd_earth.y *5;
  guidance_h_sideact.z = GUIDANCE_SIDE_THRUST - guidance_h_cmd_earth.y *5;
  VECT3_STRIM(guidance_h_sideact,0,MAX_PPRZ)
}
#endif
...
void guidance_h_from_nav(bool in_flight)
{
  ...
  if (horizontal_mode == HORIZONTAL_MODE_MANUAL) {
    ...
    stabilization_attitude_run(in_flight);
      ...
      actuators_pprz[6] = (int16_t) guidance_h_sideact.x ;
      actuators_pprz[4] = (int16_t) guidance_h_sideact.y ;
      actuators_pprz[5] = (int16_t) guidance_h_sideact.z ;
...
static void read_rc_setpoint_speed_i(struct Int32Vect2 *speed_sp, bool in_flight)
{
  if (in_flight) {
    ...
    speed_sp->x =  rc_x;//(int32_t)(((int64_t)c_psi * rc_x + (int64_t)s_psi * rc_y) >> INT32_TRIG_FRAC);
    speed_sp->y = rc_y;//(int32_t)((-(int64_t)s_psi * rc_x + (int64_t)c_psi * rc_y) >> INT32_TRIG_FRAC);
  } else {
    speed_sp->x = 0;
    speed_sp->y = 0;
  }
...
void guidance_h_guided_run(bool in_flight)
{
 ...
 /* compute x,y earth commands */
  ...
  actuators_pprz[4] = (int16_t) guidance_h_sideact.y ;
  actuators_pprz[5] = (int16_t) guidance_h_sideact.z ;
  // PRINT("commanded_THRUST2: %d  commanded_SIDE_THRUST: %d commanded_SIDE_THRUST: %d \n", guidance_h_sideact.x, guidance_h_sideact.y, guidance_h_sideact.z);
  guidance_h_cmd_earth.x = 0;
  guidance_h_cmd_earth.y = 0;
  int32_t heading_sp_i = ANGLE_BFP_OF_REAL(guidance_h.sp.heading);
  stabilization_attitude_set_earth_cmd_i(&guidance_h_cmd_earth,
